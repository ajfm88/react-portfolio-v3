...
import { loadHero, saveHero } from "./heroApi";
...
const HeroManager = () => {
  ...
  useEffect(() => {
    ...
    loadHero()
      .then((result) => {
        ...
        setPersisted(result.tagline);
        setStored(result.stored);
      })
      .catch((err) => {
        if (!cancelled) setError(err?.message || "Could not load the hero tagline.");
      })
      .finally(() => {
  ...
  // The preview runs the draft through the same parser the hero uses, so what
  // shows here is what the site will render — the markup can't drift.
  const blocks = useMemo(() => parseRichText(tagline), [tagline]);
  ...
  return (
    ...
    >
      <div className="flex flex-wrap gap-3 justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-100">Hero</h2>
        <div className="flex gap-2">
          <button
            onClick={() => setTagline(DEFAULT_HERO_TAGLINE)}
            disabled={loading || tagline === DEFAULT_HERO_TAGLINE}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-2 text-xs text-gray-500 leading-relaxed">
              The line under your name on the landing page.{" "}
